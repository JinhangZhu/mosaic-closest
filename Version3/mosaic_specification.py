...
# C:\Scripts\projects\image_processing\mosaic-closest\cifar\pics
IMG_DIR = 'C:/Scripts/projects/image_processing/mosaic-closest/images'
# IMG_DIR = './cifar/pics'
RATIO = 10
...
def load_images(height, width):
    ...
    result = []
    print('Images counts: {}'.format(len(filenames)))
...
def find_closest_image(q, shared_resource_images, resource_images_shape, shared_result, img_shape, set_height, set_width):
    ...
    while True:
        ...
        # i = random.randint(0,len(resource_images)-1)
        ...
        specified_image = tf.matching(min_img, pad)

        im_res = np.frombuffer(shared_result, dtype=np.uint8).reshape(img_shape)
        im_res[row:row+set_height, col:col+set_width, :] = specified_image
        # 
...
if __name__ == "__main__":
    str_infile = "mercy_down_down.jpg"
    str_outfile = "SPECIFICATION.jpg"
    t = time.time()
